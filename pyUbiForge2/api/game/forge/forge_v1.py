...
class BaseForgeV1(BaseForge):
    ...
    def init_iter(self) -> Generator[float, None, None]:
        ...
        database_path = os.path.join(
            CACHE_DIR, self.game_identifier, f"{self.forge_name}.forge_db"
        )
        ...
        if database is None:
            ...
            for data_file_id, (
                ...
                try:
                    ...
                    traceback.print_exc()
                    log.error(
                        f"Error loading {self.file_name} {data_file_id} {data_file_name}"
                    )
                    continue
        ...
        for data_file_id, (
            ...
            self._data_files[data_file_id] = DataFile(
                data_file_id, data_file_resource_type, data_file_name, file_storage
            )

        # populate
        yield 1.0

    def _parse_forge(self) -> Tuple[DataFileMetadata, DataFileByteLocations]:
