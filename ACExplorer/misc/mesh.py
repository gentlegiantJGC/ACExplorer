...
class BaseMesh(object):
	def __init__(self, app, model_name):
		...
		# these persist over multiple models
	...
	def group_name(self):
		...
		"""
		if self.name not in self._group_name:
			self._group_name[self.name] = -1
		self._group_name[self.name] += 1
		return '{}_{}'.format(self.name, self._group_name[self.name])
	...
	def export_obj_and_purge(self):
		...
		"""
		# write vertices
		self._obj.write(''.join(['v {} {} {}\n'.format(*vertex) for vertex in self.vertices.round(6)]))
		...
		self._obj.write(''.join(['vt {} {}\n'.format(*vertex) for vertex in self.texture_vertices.round(6)]))
		self._obj.write('# {} texture coordinates\n\n'.format(len(self.texture_vertices)))

		# write faces
		for mesh_index, mesh in enumerate(self.meshes):
			self._obj.write('g {}\n'.format(self.group_name))
			self._obj.write('usemtl {}\n'.format(self.mtl_handler.get(self.materials[mesh_index]).name))
			self._obj.write(''.join(['f {} {} {}\n'.format(*face) for face in self.faces[mesh['face_start']: mesh['face_start'] + mesh['face_count']] + self.vertex_count]))
			self._obj.write('# {} faces\n\n'.format(len(mesh['face_count'])))

		self.vertex_count += len(self.vertices)
		self.purge()

	def export_mtl_and_close(self):
		"""
