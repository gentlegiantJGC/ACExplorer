def decompressDatafile(fileTree, fileList, config, fileID, forgeFile=None):
	from misc.dataTypes import LE2DEC, LE2DEC2, BEHEX2, BE, LE2BE, LE2BE2
	from misc.decompress import decompress
	...
	if LE2BE(rawDataChunk, 0, 8) == '1004fa9957fbaa33': # if compressed
		...
		for m in xrange(compBlockCount):
			...
			if uncompressedSize == compressedSize:
				...
				uncompressedData.write(decompress(compressionType, compressedData, uncompressedSize, config))
	...
	for m in xrange(fileCount):
		...
		tempFiles.write(config, fileID2, {'game':'ACU', 'forgeFile':forgeFile, 'containerFileID':fileID, 'resourceType':fileType, 'fileName': fileName, 'dir':dir})
