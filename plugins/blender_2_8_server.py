	...
	while not command_queue.empty():
		...
		if isinstance(msg, dict) and msg.get('type', None) == 'MESH':
			...
			if 'bone_id_trm' in msg and isinstance(msg['bone_id_trm'], dict):
				...
				edit_bones = {bone_id: armature.edit_bones.new(bone_id) for bone_id in msg['bone_id_trm'].keys()}
			...
			for group_name in groups.keys():
				...
				for weight in groups[group_name]:
					mesh_object.vertex_groups[group_name].add(groups[group_name][weight], weight/255, 'REPLACE')
