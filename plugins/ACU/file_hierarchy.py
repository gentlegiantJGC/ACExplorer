...
class Plugin(BasePlugin):
    ...
    def run(self, *_):
        ...
        file_list = set(
            [
                binascii.hexlify(struct.pack("<Q", file_id)).decode("utf-8").upper()
                for file_id in pyUbiForge.temp_files.list_light_dictionary
            ]
        )
        datafile_count = 0
        ...
        for forge_file_name, forge_file_class in pyUbiForge.forge_files.items():
            for datafile_id, datafile_class in forge_file_class.datafiles.items():
                datafile_id_hex = (
                    binascii.hexlify(struct.pack("<Q", datafile_id))
                    .decode("utf-8")
                    .upper()
                )
                try:
                ...
                for file_id in datafile_class.files.keys():
                    file_id_hex = (
                        binascii.hexlify(struct.pack("<Q", file_id))
                        .decode("utf-8")
                        .upper()
                    )
                    temp_file = pyUbiForge.temp_files(
                    ...
                    dict_doc[file_id_hex][2].append(
                        [forge_file_name, datafile_id_hex, []]
                    )
                    for potential_file_id in re.findall(
                        b"(?=(.{4}[^\x00]\x00{3}))",
                        file_wrapper.read_rest(),
                        flags=re.DOTALL,
                    ):
                        potential_file_id_hex = (
                            binascii.hexlify(potential_file_id).decode("utf-8").upper()
                        )
                        if potential_file_id_hex in file_list:
                            # we have found a valid file reference
                            if (
                                potential_file_id_hex
                                not in dict_doc[file_id_hex][2][-1][2]
                            ):
                                dict_doc[file_id_hex][2][-1][2].append(
                                    potential_file_id_hex
                                )
                ...
                logging.info(
                    f"Processed {round(100*datafile_completed_count/datafile_count, 2)}% of {datafile_count} datafiles"
                )
        logging.info("Processed all files")
        with open(
            f"{pyUbiForge.CONFIG.get('dumpFolder', 'output')}/ACU_hierarchy.json", "w"
        ) as f:
