...
class Plugin(BasePlugin):
    ...
    file_type = "AC2BBF68"
    _options = [
        {"Export Method": "Wavefront (.obj)", "LOD": 0},
    ...
    def run(
        ...
        save_folder = os.path.join(
            pyUbiForge.CONFIG.get("dumpFolder", "output"), forge_file_name
        )
        ...
        if self._options[0]["Export Method"] == "Wavefront (.obj)":
            ...
            for data_block_entry_id in data_block.files:
                ...
                if data is None:
                    ...
                    continue
                if data.file_type in (
                    "0984415E",
                    "3F742D26",
                ):  # entity and entity group
                    entity: Entity = pyUbiForge.read_file(data.file)
                    if entity is None:
                        logging.warning(
                            f"Failed reading file {data.file_name} {data.file_id:016X}"
                        )
                        continue
                    for nested_file in entity.nested_files:
                        if nested_file.file_type == "EC658D29":  # visual
                            nested_file: Visual
                            if (
                                "01437462" in nested_file.nested_files.keys()
                            ):  # LOD selector
                                lod_selector: LODSelector = nested_file.nested_files[
                                    "01437462"
                                ]
                                mesh_instance_data: MeshInstanceData = lod_selector[
                                    self._options[0]["LOD"]
                                ]
                            elif (
                                "536E963B" in nested_file.nested_files.keys()
                            ):  # Mesh instance
                                mesh_instance_data: MeshInstanceData = (
                                    nested_file.nested_files["536E963B"]
                                )
                            else:
                                logging.warning(
                                    f"Could not find mesh instance data for {data.file_name} {data.file_id:016X}"
                                )
                                continue
                            if mesh_instance_data is None:
                                logging.warning(f"Failed to find file {data.file_name}")
                                continue
                            model_data = pyUbiForge.temp_files(
                                mesh_instance_data.mesh_id
                            )
                            if model_data is None:
                                logging.warning(
                                    f"Failed to find file {mesh_instance_data.mesh_id:016X}"
                                )
                                continue
                            model: mesh.BaseModel = pyUbiForge.read_file(
                                model_data.file
                            )
                            if model is None or model.vertices is None:
                                logging.warning(
                                    f"Failed reading model file {model_data.file_name} {model_data.file_id:016X}"
                                )
                                continue
                            transform = entity.transformation_matrix
                            if len(mesh_instance_data.transformation_matrix) == 0:
                                obj_handler.export(
                                    model, model_data.file_name, transform
                                )
                            else:
                                for trm in mesh_instance_data.transformation_matrix:
                                    obj_handler.export(
                                        ...
                                        numpy.matmul(transform, trm),
                                    )
                            logging.info(f"Exported {model_data.file_name}")
                else:
                    logging.info(
                        f'File type "{data.file_type}" is not currently supported. It has been skipped'
                    )
    ...
    def options(self, options: Union[List[dict], None]) -> Union[Dict[str, dict], None]:
        if options is None or (isinstance(options, list) and len(options) == 0):
            ...
            return {
                "Export Method": {"type": "select", "options": formats},
                "LOD": {"type": "select", "options": [0, 1, 2, 3, 4]},
            }
        elif isinstance(options, list):
            if len(options) == 1:
                if options[0]["Export Method"] in (
                    "Wavefront (.obj)",
                    "Collada (.dae)",
                ):
                    return {
                        "Texture Type": {
                            "type": "select",
                            "options": ["DirectDraw Surface (.dds)"],
                        }
                    }
                else:
