        ...
        if self._options[0]["Export Method"] == "Wavefront (.obj)":
            ...
            if model is not None:
                ...
                for bone in model.bones:
                    trms[bone.bone_id] = [
                        numpy.linalg.inv(bone.transformation_matrix),
                        None,
                    ]
                ...
                for bone in model.bones:
                    if bone.bone_id in bone_hierarchy:
                        parent_bone_id = next(
                            (
                                bid
                                for bid in bone_hierarchy[bone.bone_id]
                                if bid in trms
                            ),
                            None,
                        )
                        if parent_bone_id is not None:
                            trms[bone.bone_id][
                                1
                            ] = pyUbiForge.game_functions.file_types.get(
                                parent_bone_id, parent_bone_id
                            )

                bone_weights = []
                for bone_indexs, bone_weights_ in zip(
                    model.bone_numbers, model.bone_weights
                ):
                    vertex_weights = []
                    for bone_index, bone_weight in zip(bone_indexs, bone_weights_):
                        if bone_weight != 0:
                            vertex_weights.append(
                                [
                                    pyUbiForge.game_functions.file_types.get(
                                        model.bones[bone_index].bone_id,
                                        model.bones[bone_index].bone_id,
                                    ),
                                    bone_weight,
                                ]
                ...
                c.send(
                    {
                        ...
                        "bone_id_trm": {
                            ...
                            for bone_id, mats in trms.items()
                        },
