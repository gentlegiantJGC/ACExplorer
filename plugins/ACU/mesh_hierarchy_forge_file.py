...
class Plugin(BasePlugin):
	...
	def run(self, file_id: Union[str, int], forge_file_name: str, datafile_id: int, options: Union[List[dict], None] = None):
		...
		for datafile_id, datafile in pyUbiForge.forge_files[forge_file_name].datafiles.items():
			if datafile.file_type == "415D9568":
				...
				for bone_id in bone_ids:
					if bone_id in bone_hierarchy:
						bone_hierarchy[bone_id] = [v for v in bone_hierarchy[bone_id] if v in bone_ids]
					else:
